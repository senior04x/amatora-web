...
export const ObsScoreboard: React.FC<ObsScoreboardProps> = ({ streamId: propStreamId }) => {
    ...
    const resolveMatch = async () => {
      ...
      const findLiveMatch = async () => {
        ...
        if (data && data.length > 0) {
          ...
          const matchLocation = (m: any) => String(m.location || '').toLowerCase();

          // 1. Live match on matching stream field (e.g. '1-maydon', 'maydon 1', '1')
          let selectedMatch = data.find((m: any) => {
          ...
          if (!selectedMatch) {
            selectedMatch = data.find((m: any) => ['first_half', 'half_time', 'second_half'].includes(m.status));
          }

          // 3. Match matching stream field even if scheduled
          if (!selectedMatch) {
            selectedMatch = data.find((m: any) => {
              const loc = matchLocation(m);
  ...
  useEffect(() => {
    ...
    const matchSubscription = supabase
      .channel(`obs-web-match-${activeMatchId}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
    ...
    const timerSubscription = supabase
      .channel(`obs-web-timer-${activeMatchId}`)
      .on(
        'postgres_changes',
        {
          event: '*',
    ...
    const eventsSubscription = supabase
      .channel(`obs-web-match-events-${activeMatchId}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
  ...
  return (
    <div className={`obs-container ${gradientClass}`}>
      ...
      {activeEvent && (
        <div
          className={`obs-lower-third-container transformer-wrapper ${
            isEventExiting ? 'transformer-exit' : 'transformer-enter'
          }`}
        >
              ...
              <div className="obs-lt-name-container">
                {activeEvent.playerName
                  ...
                  .map((n: string, i: number) => (
                    <span key={i} className={i === 0 ? 'obs-lt-fname' : 'obs-lt-lname'}>
                      {n}{' '}
                    </span>
                  ))}
              </div>
              {activeEvent.teamLogo && <img src={activeEvent.teamLogo} className="obs-lt-team-logo" alt="" />}
            </div>
          </div>
        </div>
      )}
